...
parser.add_argument("--offset", default=1000, type=int, help="offset for prompt initinization")
...
if model_name in ['t5-small', 't5-base', 't5-large', 't5-3b']:
    from dataloader_t5 import SST2Loader, AGNewsLoader, YelpPLoader, MRPCLoader, SNLILoader, DBPediaLoader, RTELoader
    from metrics_t5 import SST2Metric, AGNewsMetric, YelpPMetric, MRPCMetric, SNLIMetric, DBPediaMetric, RTEMetric
    ...
    from dataloader import SST2Loader, AGNewsLoader, YelpPLoader, MRPCLoader, RTELoader, SNLILoader, TRECLoader, DBPediaLoader, QNLILoader, QQPLoader
    from metrics import SST2Metric, AGNewsMetric, YelpPMetric, MRPCMetric, RTEMetric, SNLIMetric, TRECMetric, DBPediaMetric, QNLIMetric, QQPMetric
...
if task_name in ['SST-2', 'Yelp', 'RTE', 'MRPC', 'chnsent', 'lcqmc', 'bq']:
    num_labels = 2
...
class LMForwardAPI:
    ...
    def calc_metric_for_multiVerbalizer(self, logits, target):
        ...
        elif self.args.task_name in ['SST-2', 'Yelp']:
            multi_verb = get_multi_verb(self.args.task_name, self.args.seed)
            multi_interest_index = []
    ...
    def eval(self, prompt_embedding=None, layer_id=None, test_data=None):
        ...
        if isinstance(test_data, DataSet):
            ...
            test_tester = Tester(data=test_data, model=self.model, metrics=self.metric, batch_size=test_batch_size,
...
if budget2 == 0:
    ...
    print('Evaluate on test data...')
    test_acc = model_forward_api.eval(test_data=test_data)
    print('Test acc: {}'.format(round(test_acc, 4)))    # if not os.path.exists(f'./{prefix}/last_epoch_results/{task_name}/{seed}'):
    #     os.makedirs(f'./{prefix}/last_epoch_results/{task_name}/{seed}')
